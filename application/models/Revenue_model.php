...
class Revenue_model extends CI_Model
{
    ...
    private function _get_datatables_query() {
        $this->db->select('order_date, count(id) as tot_order, sum(total_price) as gross_revenue, sum(shipping_cost) as shipping, sum(grand_total)
        as net_revenue, status');
        $this->db->from('orders');
        $this->db->where('payment_status', 'settlement');
        if($_POST["start_date"] && $_POST["end_date"])
        {
        $this->db->where('order_date BETWEEN "'.$_POST["start_date"] . '" and "'.$_POST["end_date"] .'"');
        }
    ...
    private function _get_datatables_query2() {
        $this->db->select("DATE_FORMAT(order_date, '%M, %Y') AS month, count(id) as tot_order, sum(total_price) as gross_revenue, sum(shipping_cost) as shipping, sum(grand_total)
        as net_revenue, status");
        $this->db->from('orders');
        $this->db->where('status', 'confirmed');
        if($_POST["start_date2"])
        {
        $this->db->where('month BETWEEN "'.$_POST["start_date2"] . '"');
        }
        $this->db->or_where('status', 'delivered');
        if($_POST["start_date2"])
        {
