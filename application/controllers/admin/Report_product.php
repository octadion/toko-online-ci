...
class Report_product extends MY_Controller
{
    ...
    function get_ajax() {
        $list = $this->report_product_model->get_datatables();
        $total_order = 0;
        $data = array();
        $no = @$_POST['start'];
        foreach ($list as $item) {
            ...
            $row[] = $item->code;
            $row[] = $item->stock;
            $row[] = indo_currency($item->base_total);
            // add html for action
        ...
        //             <button id="del'.encode_id($item->id).'" data-id="'.encode_id($item->id).'" class="swal-confirm-delete btn btn-danger btn-sm"><i class="fa fa-trash"></i> Hapus</button>';
                $total_order = $total_order + $item->base_total;
                $data[] = $row;
        }
        $output = array(
                    "draw" => @$_POST['draw'],
                    "recordsTotal" => $this->report_product_model->count_all(),
                    "recordsFiltered" => $this->report_product_model->count_filtered(),
                    "data" => $data,
    ...
    {
        $this->_display('admin/report_product/index', [
            'menu_active' => 'report_product',    
            'title' => 'Product',
